...
server <- function(input, output) {
  ...
  filtered_data <- reactive({
    Seattle_Car_Crashes %>% filter(year == input$year)
  })
  
  # Output the interactive plot with faceting and improved hover information
  output$crash_plot <- renderPlotly({
    # Create a faceted plot with custom hover info
    p <- ggplot(filtered_data(), aes(x = SEVERITYCODE, y = INJURIES, color = JUNCTIONTYPE, shape = JUNCTIONTYPE, text = paste("Severity Code:", SEVERITYCODE, "<br>Number of Injuries:", INJURIES, "<br>Junction Type:", JUNCTIONTYPE))) +
      ...
      facet_wrap(~JUNCTIONTYPE) + # Faceting by junction type
      scale_shape_manual(values = c(16, 17, 15, 4, 2)) + # Different shapes for junction types
      labs(x = "Severity Level", y = "Number of Injuries") +
      theme_minimal() +
      theme(axis.text.x = element_text(angle = 45, hjust = 1), panel.background = element_rect(fill = "lightgrey"), strip.background = element_rect(fill = "lightblue"), strip.text = element_text(size = 10)) # Style facets
