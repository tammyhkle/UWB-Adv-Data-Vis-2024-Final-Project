...
# UI
ui <- fluidPage(
  titlePanel("Seattle Car Crashes: Relationship Between Function Type, Collision Type, and Severity"),
  fluidRow(
    column(
      width = 12,
      plotlyOutput("bar_plot", width = "97%", height = "600px")
    )
  )
)
...
server <- function(input, output) {
  ...
  output$bar_plot <- renderPlotly({
    ...
    # Calculate counts of each function type, collision type, and severity level
    function_collision_severity_counts <- filtered_data %>%
      group_by(FUNCTIONTYPE, COLLISIONTYPE, SEVERITYCODE) %>%
      summarise(count = n()) %>%
      ungroup()
    
    # Create a plotly bar chart
    plot_ly(function_collision_severity_counts, x = ~FUNCTIONTYPE, y = ~count, type = 'bar', color = ~SEVERITYCODE, 
            text = ~paste("Function Type: ", FUNCTIONTYPE, "<br>Collision Type: ", COLLISIONTYPE, "<br>Count: ", count),
            hoverinfo = "text") %>%
      layout(
        title = paste("Relationship Between Function Type, Collision Type, and Severity in", input$year),
        xaxis = list(title = 'Function Type', categoryorder = 'total descending'),
        yaxis = list(title = 'Count'),
        barmode = 'stack'
      )
  })
